...
class Critic(nn.Module):
    ...
    def __call__(self, state: jax.Array, action: jax.Array) -> jax.Array:
        ...
        gru_cell = nn.GRUCell(name="critic_cell")
        h = jnp.zeros((state_action.shape[0], self.hidden_dim))
        for timestep in range(state_action.shape[1]):
            sa = state_action[:, timestep, :]
            #print("sa.shape", sa.shape)
            h,_ = gru_cell(h, sa)
            #print("h.shape", h.shape)
        out = nn.Dense(1,kernel_init=uniform_init(3e-3), bias_init=uniform_init(3e-3))(h)
...
def update_actor(
    ...
    def actor_loss_fn(params: jax.Array) -> Tuple[jax.Array, Metrics]:
        ...
        random_actions = jax.random.uniform(
            random_action_key, shape=actions.shape, minval=-1.0, maxval=1.0
        )
        new_metrics = metrics.update(
            {
                "actor_loss": loss,
                "bc_mse_policy": bc_penalty.mean(),
                "bc_mse_random": ((random_actions - batch["actions"][:,-1,:]) ** 2)
                .sum(-1)
                .mean(),
                "action_mse": ((actions - batch["actions"][:,-1,:]) ** 2).mean(),
            }
        )
...
def update_critic(
    ...
    next_actions = jnp.stack([batch["next_actions"][:,0,:],
                                ...
                                batch["next_actions"][:,2,:],
                                batch["next_actions"][:,3,:],
                                next_actions], axis=1)
    '''                                
    #print("next_actions.shape", next_actions.shape)
    # JQ: use the last time step which is the current    
    next_q = critic.apply_fn(
        critic.target_params, batch["next_states"], next_actions_
    ).min(0)
    next_q = next_q - beta * bc_penalty

    # JQ: modified to only account for current values
    target_q = batch["rewards"][:,-1] + (1 - batch["dones"][:,-1]) * gamma * next_q
...
def main(config: Config):
    ...
    init_state = np.zeros((1, config.time_steps, buffer.data["states"].shape[-1]))
    init_action = np.zeros((1, config.time_steps, buffer.data["actions"].shape[-1]))
    ...
    for epoch in trange(config.num_epochs, desc="gru_brac Epochs"):
        ...
        wandb.log(
            {"epoch": epoch, **{f"gru_brac/{k}": v for k, v in mean_metrics.items()}}
        )
